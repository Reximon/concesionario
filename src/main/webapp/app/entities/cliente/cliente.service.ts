...
export class ClienteService {
  ...
  protected convertDateFromClient(cliente: ICliente): ICliente {
    const copy: ICliente = Object.assign({}, cliente, {
      nacimiento: cliente.nacimiento != null && cliente.nacimiento.isValid() ? cliente.nacimiento.format(DATE_FORMAT) : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((cliente: ICliente) => {
        cliente.nacimiento = cliente.nacimiento != null ? moment(cliente.nacimiento) : null;
      });
    }
    return res;
  }
