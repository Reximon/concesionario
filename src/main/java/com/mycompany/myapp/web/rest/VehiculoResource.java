...
public class VehiculoResource {
    ...
    public ResponseEntity<List<Vehiculo>> getAllVehiculos(Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder) {
     ...
     /**
     * {@code GET  /vehiculos} : get disponible the vehiculos.
     *
     * @param pageable the pagination information.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of vehiculos in body.
     */
    @GetMapping("/vehiculos/disponible")
    public ResponseEntity<List<Vehiculo>> getDisponibleVehiculos(Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder) {
        log.debug("REST request to get a page of Vehiculos");
        Page<Vehiculo> page = vehiculoService.findDisponible(pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(uriBuilder.queryParams(queryParams), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
